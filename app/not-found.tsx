...
export default function NotFound() {
  return <main id="main" tabIndex={-1} className="container not-found">
    ...
    <nav aria-label="代表的な制作" className="recovery-grid">{featuredProjects.map((p, i) => <a key={p.id} href={projectPath(p.slug)}><span className="eyebrow">0{i + 1} / Selected Work</span><h2>{p.title} <span aria-hidden="true">→</span></h2><p>{p.description.text}</p></a>)}</nav>
  </main>;
}
