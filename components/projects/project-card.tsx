...
export function ProjectCard({ project }: { project: Project }) {
  ...
  return <article className={featured ? "project-row" : "secondary-row"} data-repository={project.id}>
    ...
    <div className="project-copy">{featured && <ScreenPreview id={project.id} />}<p className="repo-name">{project.name}</p><h3 id={id} tabIndex={-1}>{featured ? <a href={projectPath(project.slug)}>{project.title}</a> : project.title}</h3><p>{project.description.text}</p>
